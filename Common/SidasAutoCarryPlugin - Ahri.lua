...
class 'Plugin' -- {
	...
	function Plugin:__init() 
		...
		combo:AddCustomCast(_R, function(target) return ((DamageCalculation.CalculateRealDamage(target) > target.health) or ((getDmg("R", target, myHero) * 3) > target.health) or ((GetTickCount() - rTick) < 10000)) end)
