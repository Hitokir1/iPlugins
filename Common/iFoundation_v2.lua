...
class 'Caster' -- {
	...
	function Caster:CastMec(Target, minTargets)
		if self:Ready() then return false end
...
class 'AutoPotion' -- {
	...
	function AutoPotion:__init()
		self.Menu = scriptConfig("iFoundation: Auto Potions", "ipotions" .. myHero.charName)
...
class "Drawing" -- {
	...
	function Drawing:_AddSkill(sspell, rrange)
		...
		self.Menu:addParam("colorReady" .. tempSpell, "Ready " .. tempSpell .. " Color", SCRIPT_PARAM_COLOR, {40, 71,149,38})
		self.Menu:addParam("colorNotReady" .. tempSpell, "Not Ready " .. tempSpell .. " Color", SCRIPT_PARAM_COLOR, {40, 177, 0, 0})
	...
	function Drawing:OnDraw()
		for name,d in pairs(self.queue) do 
			if string.find(name, "Skill") and self.Menu["draw" .. d.spellString] then
				local tempColor = _ColorARGB.FromTable(self.Menu["colorNotReady" .. d.spellString])
				if myHero:CanUseSpell(d.spell) == READY then 
					tempColor = _ColorARGB.FromTable(self.Menu["colorReady" .. d.spellString])
				end
...
class "Monitor" -- {
	...
	function Monitor:__init() 
		self.Menu = scriptConfig("iFoundation: Monitoring", "imonitor" .. myHero.charName)
	...
	function Monitor:OnTick() 
		if self.Menu.drawAlly and self:_GetLowAlly() ~= nil then 
			_Message.AddMessage("Monitor: Player dropped below threshold health", _ColorARGB.Red) 
		end 
	end 
...
class 'AutoShield' -- {
	...
	function AutoShield:__init(range, ShieldSpell)
		...
		self.Menu = scriptConfig("iFoundation: AutoShield", "iautoshield" .. myHero.charName)
		...
		if self.ShieldSpell.spellType ~= SPELL_SELF or AutoCarry.selfOverride then 
...
class 'ComboLibrary' -- {
	...
	function ComboLibrary:__init()
		self.casters = {}
		self.Menu = scriptConfig("iFoundation: Combo", "icombo" .. myHero.charName)
	end 

	function ComboLibrary:AddCasters(table, addKS)
		self.addKS = addKS or true
...
class 'AutoKS' -- {
	...
	function AutoKS:__init()
		...
		self.target = nil
		self.caster = nil
		self.Menu = scriptConfig("iFoundation: Auto-KS", "iautoks" .. myHero.charName)
		self.Menu:addParam("hotkey", "KS HotKey", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("T"))
		AddTickCallback(function(obj) self:OnTick() end)
	end 
	...
	function AutoKS:Find()
		for _, enemy in pairs(_Heroes.GetObjects(ENEMY, math.huge)) do
	...
	function AutoKS:OnTick() 
		...
		if self.target and not self.target.dead then
			if self.Menu["message" .. self.caster.string] then
				local message = "Performing!"
				if not self.Menu["use" .. self.caster.string] then
					message = "Available! Press hotkey!"
				end 
				_Message.AddMessage("AutoKS: " .. message, _ColorARGB.Green)
			end 
			if self.Menu["use" .. self.caster.string] or self.Menu.hotkey then
...
class 'AutoFarm' -- {
	...
	function AutoFarm:__init() 
		self.Menu = scriptConfig("iFoundation: Auto-Farm", "iautofarm" .. myHero.charName)
		self.casters = {}
	...
	function AutoFarm:_AddCaster(caster) 
		local tempSpell = SpellToString(caster.spell)
		self.Menu:addParam("sepks" .. tempSpell, "-- " .. tempSpell .. " Options --", SCRIPT_PARAM_INFO, "")
		self.Menu:addParam("use" .. tempSpell, "Use " .. tempSpell .. " for LastHit", SCRIPT_PARAM_ONOFF, false)
		table.insert(self.casters, {casterInstance = caster, string = tempSpell})
	end 
	...
	function AutoFarm:GetCaster() 
		for _, caster in pairs(self.casters) do  
			if self.Menu["use" .. caster.string] then 
	...
	function AutoFarm:DisableOptions(except)
		for _, caster in pairs(self.casters) do 
			if self.Menu["use" .. caster.string] and caster.string ~= except then 
				self.Menu["use" .. caster.string] = false 
			end 
		end 
	end 
